...
//    }
  ...
  function addToCart(productId) {
    ...
    if (product) {
      cart.push(product);
      localStorage.setItem("cart", JSON.stringify(cart)); // Salva il carrello in localStorage
      console.log("Carrello aggiornato:", cart); // Debug: Mostra il carrello aggiornato
      alert(`${product.name} è stato aggiunto al carrello!`);
    }
  }
  ...
  function displayCart() {
    ...
    cartItems.forEach((item, index) => {
      ...
      cartHTML += `
        <div class="cart-item">
          <img src="image/${item.name.toLowerCase().replace(/ /g, "-")}.jpg" alt="${item.name}">
